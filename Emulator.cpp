...
NAME
	...
	bool emulator::InsertMemory - insert memory into emulator

SYNOPSIS

	bool emulator::InsertMemory(int a_loc, int a_content);

		a_loc --> location of contents
		a_content --> content to insert into location

DESCRIPTION

	Checks to see if location in memory is free, if it is it puts it there

RETURNS

	Returns a BOOL value true if the insertion worked, false if the spot in memory was already taken
*/
bool emulator::InsertMemory(int a_loc, int a_content) {
	if (m_memory[a_loc] != 0) {
		return false;
...
NAME
	...
	bool emulator::RunProgram - run the program as stored in memory
...
DESCRIPTION
	...
	Goes through memory and runs commands in it

RETURNS

	Returns a BOOL - true if it went with no errors, otherwise false
...
NAME
	...
	void emulator::Read - read into memory

SYNOPSIS

	void emulator::Read(int a_location);

		a_location --> location in memory to read to

DESCRIPTION

	Accepts an int as input and stores it in memory
